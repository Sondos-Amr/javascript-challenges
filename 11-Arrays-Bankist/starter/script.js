...
const checkDogs2 = function (dogsJulia, dogsKate) {
  ...
  juliaDogs.splice(-2);
  const allDods = juliaDogs.concat(dogsKate);
  allDods.forEach((dogAge, i) => {
    if (dogAge >= 3) {
      console.log(
        `Dog number ${i + 1} is an adult, and is ${dogAge} years old`
      );
...
const calcAverageHumanAge2 = function (dogsAge) {
  const humanAge = dogsAge
    ...
    .filter(dog => dog >= 18);
  const average = humanAge.reduce((acc, dog) => acc + dog, 0) / humanAge.length;
  return average;
};
